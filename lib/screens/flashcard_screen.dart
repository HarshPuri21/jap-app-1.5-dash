...
enum _CardKind { vocab, kanji, kana }
...
class _Card {
  ...
  String get itemId {
    switch (kind) {
      case _CardKind.vocab:
        return vocabItemId(vocab!.jp);
      case _CardKind.kanji:
        return kanjiItemId(kanji!.kanji);
      case _CardKind.kana:
        return kanaItemId(kana!.character);
    }
  }
}
...
class _FlashcardScreenState extends State<FlashcardScreen> {
  ...
  void _buildDeck() {
    ...
    final kanaCards = DataService.instance
        .kanaQuizPool('mixed')
        .map((k) => _Card.kana(k))
        .toList();
    switch (_filter) {
      ...
      case 'kana':
        ...
        break;
      default:
        _deck = [...vocabCards, ...kanjiCards, ...kanaCards];
    }
    _deck.shuffle();
    _flipped.clear();
    _known = 0;
          ...
          child: AnimatedSwitcher(
            ...
            child: KeyedSubtree(
              ...
              child: card.kind == _CardKind.vocab
                  ...
                  : card.kind == _CardKind.kanji
                      ? _kanjiFace(context, card.kanji!, flipped)
                      : _kanaFace(context, card.kana!, flipped),
            ),
          ),
        ),
      ),
  ...
  Widget _kanaFace(BuildContext context, KanaEntry k, bool flipped) {
    final t = context.tokens;
    if (!flipped) {
      return Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          FittedBox(
            fit: BoxFit.scaleDown,
            child:
                Text(k.character, style: t.text.jp(92, weight: FontWeight.w700)),
          ),
          SizedBox(height: t.spacing.lg),
          Text('Tap to reveal reading', style: t.text.caption),
        ],
      );
    }
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Text(k.character, style: t.text.jp(44, weight: FontWeight.w700)),
        SizedBox(height: t.spacing.sm),
        Text(
          k.romaji.isEmpty || k.romaji == '—' ? k.sound : k.romaji,
          textAlign: TextAlign.center,
          style: t.text.display.copyWith(
            color: t.colors.accent,
            fontSize: 24,
        ...
        SizedBox(height: t.spacing.xxs),
        Text(
          '${k.isHiragana ? 'Hiragana' : 'Katakana'} · ${kanaGroupLabel(k.group)}',
